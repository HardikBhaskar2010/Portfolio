...
/**
 * Wraps an element and registers its bounding box with the HighlightStore.
 * The ScrollOrb will track the nearest HighlightPoint as you scroll.
 */
export function HighlightPoint({ id, children, className, as: Component = 'div' }: HighlightPointProps) {
  ...
  useEffect(() => {
    if (ref.current) {
      addHighlight(id, ref.current);
    }
    return () => removeHighlight(id);
  }, [id, addHighlight, removeHighlight]);
